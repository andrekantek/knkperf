...
gint32 buildQueuePacketUdp(guint8 queue, libnet_t *lnet, parsed_options_t* options)
{
    ...
    int j = 0;
    u_int8_t payload[options->tx.pktsize];
    ...
    u_int8_t ip_frag = 0;
    u_int8_t ip_ttl = 64;
    u_int16_t ip_proto = IPPROTO_UDP;
...
gint32 buildQueuePacketIgmp(guint8 queue, libnet_t *lnet, parsed_options_t* options)
{
    //==========================================================================
    int len;
    u_int8_t *mac_src = libnet_hex_aton((options->vlan.macsrc)->str, &len);
    u_int8_t new_byte = (mac_src[5]+queue);
    mac_src[5] = (u_int8_t)new_byte;
    u_int8_t *mac_dst = libnet_hex_aton((options->vlan.macdst)->str, &len);
    ...
    u_int8_t ip_frag = 0;
    u_int8_t ip_ttl = 1;
    u_int16_t ip_proto = IPPROTO_IGMP;
    u_int16_t ip_chksum = 0;
    u_int8_t *ip_payload = NULL;
    u_int32_t ip_payload_s = 0;
